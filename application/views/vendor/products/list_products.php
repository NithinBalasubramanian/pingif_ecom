  ...
  <div class="card shadow mb-4">
    ...
    <table id="dataTable" class="table table-bordered table-striped">
            ...
            <?php $cus_id=$this->session->userdata('id');
             $product=$this->Admin_model->table_column($tablename='shop_products','customer_id',$cus_id);
            if(count($product)>0){
              $i=1;
              foreach($product as $p_row){
                ...
                <?php  }
                    ...
                    <td><?php if($p_row['status'] == '1'){ ?><button class="btn btn-sm btn-success status status_<?php echo $p_row['id']; ?>" data-val="1" data-id="<?php echo $p_row['id']; ?>">Live</button><?php }else{ ?><button class="btn btn-sm btn-danger status status_<?php echo $p_row['id']; ?>" data-val="0" data-id="<?php echo $p_row['id']; ?>">Unlive</button><?php } ?></td>
                  <td>
                  <a href="<?php echo base_url();?>View_vendor/products/edit_products/<?php echo $p_row['id'];?>" class="btn btn-sm btn-info">Edit</a>
                  <a href="javascript:void(0)" class="btn btn-sm btn-danger per_delete" data-id="<?php echo $p_row['id']; ?>" data-table="shop_products">Delete</a></td>
                </tr>
              <?php $i++; } ?>
            <?php } ?>
                </tbody>
